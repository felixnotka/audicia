...
Audicia's core output is a set of least-privilege RBAC manifests derived from
real API usage. The generated policy represents what a subject _actually needs_,
not what someone guessed at provisioning time. This page explains what gets
generated, how, and what you can do with it.

## What Gets Generated

For each subject (ServiceAccount, User, or Group) that appears in the audit log,
Audicia produces ready-to-apply Kubernetes RBAC manifests:
...
The [strategy engine](../components/strategy-engine.md) applies configurable
knobs to shape the RBAC output:
...
A ServiceAccount `backend` in namespace `my-team` accesses pods
(get/list/watch), configmaps (get/watch), and pods/exec (create). The generated
policy:
...
This is the minimal RBAC the SA needs based on observed behavior. Compare it
against the SA's current permissions via
[compliance scoring](compliance-scoring.md) to see what's excessive.
...
The quality of the generated policy depends on the observation window. Short
windows may miss infrequent operations:
...
Use `retentionDays` on the AudiciaSource to control how long rules are kept
...
Group identities are captured from the audit log but are not used for binding
generation by default. The generated bindings reference the subject
(ServiceAccount or User) directly.

## Relationship to Compliance Scoring

RBAC generation and [compliance scoring](compliance-scoring.md) are
complementary:
...
Together they form a closed loop: compliance scoring identifies the problem
(overprivilege), and RBAC generation provides the solution (least-privilege
policy).
