...
Audicia is the best tool for generating least-privilege RBAC policies from
Kubernetes audit logs. Focus on doing one thing extremely well: observe what
your workloads do, generate correct policies, and show you where you're
overprivileged.
...
1. **Never auto-apply.** Audicia generates recommendations. Humans (or reviewed
   GitOps pipelines) apply them.
2. **CRD-native.** Every output is a Kubernetes resource. No side-channel files,
   no external databases required.
3. **Minimal operator permissions.** Audicia itself should be a model of
   least-privilege.
4. **Do one thing well.** Focus on audit log → RBAC policy generation. Don't try
   to be a platform.
